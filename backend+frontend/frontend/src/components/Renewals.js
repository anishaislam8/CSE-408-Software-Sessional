...
class NTTNRenewals extends React.Component {
  ...
  componentDidMount() {
    ...
    apiUrl = "http://localhost:7000/nttn/renewal";
    ...
    axios
      ...
      .then((response) => {
        this.setState(
          ...
          () => {
            let pageCountVal = this.state.renewals
              ? Math.ceil(this.state.renewals.length / pageSize)
              : 0;
    ...
    axios
      ...
      .then((response) => {
        this.setState({ filteredrenewals: response.data.data }, () => {
          //console.log("here");
          let pageCountVal = this.state.filteredrenewals
            ? Math.ceil(this.state.filteredrenewals.length / pageSize)
            : 0;
          let pagesVal = _.range(1, pageCountVal + 1);
      ...
      this.setState(
        {
          ...
          filteredrenewals: this.state.renewals.filter((renewal) => {
            return (
              this.getIspName(renewal.isp_id)
                .toLowerCase()
                .includes(e.target.value.toLowerCase()) ||
              this.getUnionName(renewal.union_id)
                .toLowerCase()
                .includes(e.target.value.toLowerCase()) ||
                this.getISP(renewal.isp_id).license_id.toLowerCase().includes(e.target.value.toLowerCase()) ||
                this.getConnection(this.getISP(renewal.isp_id).physical_connection_details[0].connection_id).head_office_telephone.includes((e.target.value).toLowerCase()) ||
                this.getConnection(this.getISP(renewal.isp_id).physical_connection_details[0].connection_id).head_office_mobile.includes((e.target.value).toLowerCase()) ||
                this.getConnection(this.getISP(renewal.isp_id).physical_connection_details[0].connection_id).office_telephone.includes((e.target.value).toLowerCase()) ||
                this.getConnection(this.getISP(renewal.isp_id).physical_connection_details[0].connection_id).office_mobile.includes((e.target.value).toLowerCase()) ||
                this.getConnection(this.getISP(renewal.isp_id).physical_connection_details[0].connection_id).contact_person_telephone.includes((e.target.value).toLowerCase()) ||
                this.getConnection(this.getISP(renewal.isp_id).physical_connection_details[0].connection_id).contact_person_mobile.includes((e.target.value).toLowerCase())
            );
          }),
        },
        () => {
  ...
  handleChangeRatingOrderAll(e) {
    this.setState(
      ...
      () => {
        if (e.target.value === "1") {
          this.setState((prevstate) => ({
            filteredrenewals: prevstate.filteredrenewals.sort(
              (a, b) => a.rating - b.rating
            ),
          }));
        } else {
  ...
  handleChangeArrivalTimeOrderAll(e) {
    this.setState(
      ...
      () => {
        if (e.target.value === "1") {
          this.setState((prevstate) => ({
            filteredrenewals: prevstate.filteredrenewals.sort((a, b) =>
              a.request_arrival_time.localeCompare(b.request_arrival_time)
            ),
          }));
        } else {
    ...
    this.setState(
      ...
      () => {
        ...
        const newPaginatedData =
          this.state.filteredrenewals.length === 0
            ? []
            : _(this.state.filteredrenewals)
                .slice(startIndex)
                .take(pageSize)
                .value();
  ...
  render() {
    return (
      <div className="container">
        ...
        <br></br>
        <center>
        <center><h3 className="display-6" style={{"marginBottom" : 50, "marginTop" : 50}}>Renewal Requests From ISPs</h3></center>
          <br></br>
        </center>
        <div className="row">
          <div className="col">
          ...
          <div className="col">
            <Button
              ...
              style={{ marginBottom: 20, marginRight: 10, width: 200 }}
            >
              <BsIcons.BsCardChecklist size={30} /> Show All Request
            </Button>
          </div>
          <div className="col">
            <input
        ...
        <table className="table table-bordered table-striped">
          ...
          <tbody>
            {this.state.paginatedData.length > 0 &&
              this.state.paginatedData.map((renewal, index) => {
                return (
                  <Renewal
                    key={renewal._id}
          ...
          <ul className="pagination">
            ...
            >
              <p
                className="page-link"
                onClick={() =>
                  this.paginationrenewals(this.state.currentPage - 1)
                }
                aria-label="Previous"
              >
            ...
            {this.state.pages.map((page) => {
              return (
                ...
                >
                  <p
                    className="page-link"
                    onClick={() => this.paginationrenewals(page)}
                  >
                    {page}
                  </p>
                </li>
