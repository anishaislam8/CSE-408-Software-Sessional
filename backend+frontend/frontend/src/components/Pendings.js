      ...
      if(e.target.value===""){
        ...
        this.setState({
          ...
          filteredPendings : this.state.pendings.filter((pending) => {
            return this.getIspName(pending.isp_id).toLowerCase().includes((e.target.value).toLowerCase()) || 
            this.getUnionName(pending.union_id).toLowerCase().includes((e.target.value).toLowerCase())
          })
        }, () => {
          let pageCountVal = this.state.filteredPendings ? Math.ceil(this.state.filteredPendings.length / pageSize) : 0;
          let pagesVal = _.range(1, pageCountVal + 1);
    ...
    handleChangeRatingOrder(e){
      ...
      handleChangeRatingOrderAll(e){
        this.setState({
          ...
          if(e.target.value === "1"){
            this.setState((prevstate) => ({filteredPendings : prevstate.filteredPendings.sort((a,b) => a.rating - b.rating)}))
          } else {
            this.setState((prevstate) => ({filteredPendings : prevstate.filteredPendings.sort((a,b) => b.rating - a.rating)}))
          }
    ...
    render() {
        return(
            <div className="container">
              ...
              <br></br>
              <br></br>
                <center><h3 style={{"margin":20}}>Pending Requests from ISPs</h3><br></br></center>
