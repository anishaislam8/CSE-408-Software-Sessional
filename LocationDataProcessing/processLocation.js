const fs = require('fs');
let divisions = [{"id":"1","division_id":"1","name":"Comilla","bn_name":"কুমিল্লা","lat":"23.4682747","lon":"91.1788135","url":"www.comilla.gov.bd"},
...
let updated_divisions = [];

for (let i = 0; i < divisions.length; i++){
    updated_divisions.push(JSON.parse(JSON.stringify(divisions[i])));
}
let updated_divisions2 = [];
for (let i = 0; i < updated_divisions.length; i++){
    updated_divisions2.push({
        name : updated_divisions[i].name,
        district_id : updated_divisions[i].id,
        divison_id : updated_divisions[i].division_id
    });
}

let finalJson = JSON.stringify(updated_divisions2).toString();

let finalJSONString = finalJson.replace(/},{/g, "}{");
fs.writeFileSync('districts.txt', finalJSONString);