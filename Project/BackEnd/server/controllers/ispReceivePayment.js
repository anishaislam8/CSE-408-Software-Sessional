...
const handlePaymentDone = async (request, response) => {
  ...
  try {
    ...
    if (package.package_type !== 1) {
      return response.status(400).send({
        message: "Package type invalid",
        data: [],
      });
    }
    if (user.connection_status === false) {
      user.connection_status = true;
      user.connection_establishment_date = new Date();
      //user.expiration_date = new Date() + package.duration;
    }
    ...
    return response.status(200).send({
      message: "Contract insertion Successful and Payment and User updated",
      data: data + "\n" + updateduser + "\n" + updatedpayment,
    });
